import sys
# Nuevo: Importar os para manejo de rutas de archivo
import os
...
from PyQt5.QtWidgets import (
    ...
    QCheckBox, QGroupBox, QTabWidget, QTextEdit, QComboBox, QMessageBox # Necesario para el mensaje de error
)
from PyQt5.QtCore import Qt, QTimer, QSize
from PyQt5.QtGui import QIcon, QPixmap, QPalette, QBrush, QPainter # QPainter es crucial


# ===================================================================
# CLASE MODIFICADA PARA FONDO (BackgroundWidget)
# ===================================================================
class BackgroundWidget(QWidget):
    def __init__(self, image_path, parent=None):
        super().__init__(parent)
        
        # Construir la ruta absoluta (más robusta)
        ...
        if self.background_image.isNull():
             QMessageBox.critical(
                 self, 
                 "Error de Imagen", 
                 f"No se pudo cargar la imagen de fondo.\nVerifique que 'imagenes/fondo.png' exista.\n"
                 f"Ruta esperada: {absolute_path}"
               )
...
class VentanaPrincipal(QMainWindow):
    ...
    def cargar_equipos(self):
        # Nota: Asumo que esta parte funciona correctamente con tu clase NBADataExtractor
        ...
        if lista_equipos:
            ...
            if len(self.nombre_equipos) > 0:
                self.nombre_local_cb.setCurrentIndex(1)
                self.nombre_visitante_cb.setCurrentIndex(2)
    ...
    def crear_pestaña_principal(self):
        ...
        self.nombre_local_cb.setStyleSheet("font-size: 18px; font-weight: bold;")
        layout_local.addWidget(self.nombre_local_cb)
        ...
        self.nombre_visitante_cb.setStyleSheet("font-size: 18px; font-weight: bold;")
        self.nombre_visitante_cb.currentIndexChanged.connect(self.actualizar_tablero)
        layout_visitante.addWidget(self.nombre_visitante_cb)
        ...
        # -------------------------------
        # Usamos BackgroundWidget para el lado derecho
        lado_derecho_widget = BackgroundWidget("imagenes/fondo.png")
        lado_derecho = QVBoxLayout(lado_derecho_widget)
        
        # Tamaño de los iconos para las pelotas
        icon_size = QSize(60, 60) # Aumentado a 60x60 para mejor visibilidad

        # Tablero Local
        self.tablero_local = QLabel(f"Local: {self.marcador_local}")
        self.tablero_local.setAlignment(Qt.AlignCenter)
        self.tablero_local.setStyleSheet("font-size: 48px; font-weight: bold; color: black; padding: 20px;")
        lado_derecho.addWidget(self.tablero_local)

        # Botones marcador local - MODIFICADO CON IMÁGENES
        botones_local = QHBoxLayout()
        
        # Botón +1 (pelota1.png)
        btn_plus_1_local = QPushButton()
        btn_plus_1_local.setIcon(QIcon("imagenes/pelota1.png"))
        btn_plus_1_local.setIconSize(icon_size)
        btn_plus_1_local.clicked.connect(partial(self.sumar_marcador, "local", 1))
        botones_local.addWidget(btn_plus_1_local)
        
        # Botón +2 (pelota2.png)
        btn_plus_2_local = QPushButton()
        btn_plus_2_local.setIcon(QIcon("imagenes/pelota2.png"))
        btn_plus_2_local.setIconSize(icon_size)
        btn_plus_2_local.clicked.connect(partial(self.sumar_marcador, "local", 2))
        botones_local.addWidget(btn_plus_2_local)
        
        # Botón +3 (pelota3.png)
        btn_plus_3_local = QPushButton()
        btn_plus_3_local.setIcon(QIcon("imagenes/pelota3.png"))
        btn_plus_3_local.setIconSize(icon_size)
        btn_plus_3_local.clicked.connect(partial(self.sumar_marcador, "local", 3))
        botones_local.addWidget(btn_plus_3_local)
        ...
        # Tablero Visitante
        self.tablero_visitante = QLabel(f"Visitante: {self.marcador_visitante}")
        self.tablero_visitante.setAlignment(Qt.AlignCenter)
        self.tablero_visitante.setStyleSheet("font-size: 48px; font-weight: bold; color: black; padding: 20px;")
        lado_derecho.addWidget(self.tablero_visitante)

        # Botones marcador visitante - MODIFICADO CON IMÁGENES
        botones_visitante = QHBoxLayout()
        
        # Botón +1 (pelota1.png)
        btn_plus_1_visitante = QPushButton()
        btn_plus_1_visitante.setIcon(QIcon("imagenes/pelota1.png"))
        btn_plus_1_visitante.setIconSize(icon_size)
        btn_plus_1_visitante.clicked.connect(partial(self.sumar_marcador, "visitante", 1))
        botones_visitante.addWidget(btn_plus_1_visitante)
        
        # Botón +2 (pelota2.png)
        btn_plus_2_visitante = QPushButton()
        btn_plus_2_visitante.setIcon(QIcon("imagenes/pelota2.png"))
        btn_plus_2_visitante.setIconSize(icon_size)
        btn_plus_2_visitante.clicked.connect(partial(self.sumar_marcador, "visitante", 2))
        botones_visitante.addWidget(btn_plus_2_visitante)
        
        # Botón +3 (pelota3.png)
        btn_plus_3_visitante = QPushButton()
        btn_plus_3_visitante.setIcon(QIcon("imagenes/pelota3.png"))
        btn_plus_3_visitante.setIconSize(icon_size)
        btn_plus_3_visitante.clicked.connect(partial(self.sumar_marcador, "visitante", 3))
        botones_visitante.addWidget(btn_plus_3_visitante)
        ...
        self.label_cronometro.setStyleSheet("font-size: 32px; font-weight: bold; margin-top:20px; color: black;")
        ...
        btn_iniciar.setIcon(QIcon("imagenes/play.png"))
        btn_iniciar.setIconSize(QSize(40, 40))
        btn_iniciar.clicked.connect(self.iniciar_cronometro)
        botones_crono.addWidget(btn_iniciar)

        btn_pausar = QPushButton()
        btn_pausar.setIcon(QIcon("imagenes/pause.png"))
        btn_pausar.setIconSize(QSize(40, 40))
        btn_pausar.clicked.connect(self.pausar_cronometro)
        botones_crono.addWidget(btn_pausar)

        btn_reiniciar = QPushButton()
        btn_reiniciar.setIcon(QIcon("imagenes/restart.png"))
        btn_reiniciar.setIconSize(QSize(40, 40))
        ...
        self.label_faltas_local.setStyleSheet("font-size: 22px; font-weight: bold; color: black;")
        ...
        self.label_faltas_visitante.setStyleSheet("font-size: 22px; font-weight: bold; color: black;")
        ...
        layout_principal.addWidget(lado_derecho_widget, 1)

        return widget
    # -------------------------------
    # Planilla de jugadores (el resto de métodos se mantienen igual)
    # -------------------------------
    ...
    def crear_pantalla_estadisticas(self):
        ...
        self.area_estadisticas.setText("En este QTextEdit voy a probar meter datos de la API")
    ...
    def verificar_faltas(self, fila, tabla, equipo):
        contador = sum(
            1
            for col in range(2, 7)
            if isinstance(tabla.cellWidget(fila, col), QCheckBox)
            and tabla.cellWidget(fila, col).isChecked()
        )
        if contador <= 2:
    ...
    def actualizar_faltas_totales(self, equipo):
        ...
        if equipo == "local":
            ...
            self.label_faltas_local.setText(f"Faltas {nombre_local_cb}: {self.faltas_local}/5")
        else:
            self.faltas_visitante = total
            self.label_faltas_visitante.setText(f"Faltas {nombre_visitante_cb}: {self.faltas_visitante}/5")

    def resetear_faltas_totales(self):
        self.faltas_local = 0
        self.faltas_visitante = 0
        nombre_local = self.nombre_local_cb.currentText()
        nombre_visitante = self.nombre_visitante_cb.currentText()
        self.label_faltas_local.setText(f"Faltas {nombre_local}: 0/5")
        self.label_faltas_visitante.setText(f"Faltas {nombre_visitante}: 0/5")
    ...
    def actualizar_tablero(self):
        nombre_local_cb = self.nombre_local_cb.currentText()
        nombre_visitante_cb = self.nombre_visitante_cb.currentText()
        self.tablero_local.setText(f"{nombre_local_cb}: {self.marcador_local}")
        self.tablero_visitante.setText(f"{nombre_visitante_cb}: {self.marcador_visitante}")
        self.label_faltas_local.setText(f"Faltas {nombre_local_cb}: {self.faltas_local}/5")
        self.label_faltas_visitante.setText(f"Faltas {nombre_visitante_cb}: {self.faltas_visitante}/5")
...
if __name__ == "__main__":
    ...
    # 🏀 NUEVO: Establecer el icono en la barra de tareas
